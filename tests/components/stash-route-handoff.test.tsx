...
vi.mock("@/components/stash-scan/screenshot-overlay", () => ({ ScreenshotOverlay: () => null, OverlayLegend: () => null }));
  ...
  return <button onClick={() => {
    store.setSession({ images: [{ id: "test", url: "/test.png" }], results: [{
      ...
      cells: [0, 1].map((x) => ({ x: x * 50, y: 0, width: 50, height: 50, slotsWide: 1, slotsHigh: 1,
        empty: false, confidence: "high" as const, matches: [{ itemId: "test-item", shortName: "Test", rotated: false, score: 1 }] })),
    }] });
    store.setAssignments({ "0:0": "test-item", "0:1": "test-item" });
  }}>Seed scan</button>;
}
function renderScan() {
  render(<StashScanStoreProvider><SeedScan /><ScanWithCommit /></StashScanStoreProvider>);
  fireEvent.click(screen.getByText("Seed scan"));
}
afterEach(cleanup);

describe("route stash handoff", () => {
  it.each(["Load into calculator", "Use this stash"])("%s saves inventory and source before navigating", (label) => {
  ...
  it("updates an existing stash and switches back from market to stash", () => {
    ...
    renderScan();
    fireEvent.click(screen.getByRole("button", { name: "Update stash" }));
    expect(localStorage.getItem("autoSelectSource")).toBe("stash");
    expect(push).toHaveBeenCalledWith("/");
  });
  it("saves an unreachable stash, warns and preserves the current calculator slots", () => {
    ...
    renderScan();
    fireEvent.click(screen.getByRole("button", { name: "Use this stash" }));
    expect(JSON.parse(localStorage.getItem("stashInventory")!).items["test-item"].count).toBe(2);
