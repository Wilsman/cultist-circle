...
export function ComboRow({ combo }: { combo: SacrificeCombo }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 rounded-xl bg-white/50 dark:bg-slate-800/40 border border-slate-200/50 dark:border-slate-700/50 p-3 transition-all hover:bg-white/80 dark:hover:bg-slate-800/60 hover:border-slate-300 dark:hover:border-slate-600">
      <div className="flex flex-wrap items-center gap-2 flex-1">
        {combo.ingredients.map((ingredient, idx) => (
          <div key={idx} className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900/60 rounded-lg px-2 py-1 ring-1 ring-black/5 dark:ring-white/5">
              <Image
              ...
              <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">
                {ingredient.count}×{" "}
                <span className="hidden xs:inline">{ingredient.name}</span>
              </span>

              {ingredient.vendor && (
                <div
                  className="flex items-center gap-1 pl-1.5 border-l border-slate-200 dark:border-slate-700/50 ml-1 group/vendor cursor-help"
                  title={`Buy from ${ingredient.vendor.name}`}
                >
                  <Image
                  ...
                  />
                  <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 group-hover/vendor:text-slate-700 dark:group-hover/vendor:text-slate-200 transition-colors">
      ...
      <div className="flex items-center justify-end sm:justify-start mt-1 sm:mt-0 pl-1">
        <span
