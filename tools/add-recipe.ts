...
async function searchItem(searchTerm: string): Promise<ItemResult[]> {
  ...
  `;
    ...
    try {
        ...
        let json: { data?: { pvp: ItemResult[]; pve: ItemResult[] }; errors?: unknown[] };
        try {
...
async function lookupItem(
    ...
    if (results.length === 0) {
        ...
        const iconUrl =
            manual || "https://assets.tarkov.dev/unknown-item-512.webp";
    ...
    const choice = await prompt(`   Select (1-${Math.min(maxShow, results.length)}): `);
...
async function main() {
    ...
    console.log("This tool helps you add new recipes by looking up items in the API.\n");

    // Collect input items
    const inputStr = await prompt("Enter INPUT items (comma-separated):\n> ");
    const inputNames = inputStr.split(",").map((s) => s.trim()).filter(Boolean);
    ...
    const outputNames = outputStr.split(",").map((s) => s.trim()).filter(Boolean);
    ...
    recipeLines.push("    {");
    recipeLines.push(
        `        requiredItems: [${inputItems.map((i) => `"${i.displayName}"`).join(", ")}],`
    );
    ...
    if (addToFiles.toLowerCase() === "y") {
        try {
            ...
            const arrayStartMatch = recipesContent.match(/export const tarkovRecipes:\s*Recipe\[\]\s*=\s*\[/);
