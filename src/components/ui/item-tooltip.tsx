...
export function ItemTooltip({ item, children, iconUrl }: ItemTooltipProps) {
  ...
  const getBestTraderPrice = () => {
    ...
    const best = item.buyFor.reduce(
      (max, offer) => (offer.priceRUB > (max?.priceRUB || 0) ? offer : max),
      item.buyFor[0]
    );
    return best;
  };
  ...
  const isTraderFleaMarket =
    bestTrader?.vendor?.normalizedName?.toLowerCase() === "flea-market";
