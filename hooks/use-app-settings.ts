...
export function useAppSettings() {
  const [gameMode, setGameMode] = useLocalStorageString<GameMode>(
    GAME_MODE_STORAGE_KEY,
    "pvp",
    GAME_MODES,
  );
  ...
  // Threshold
  const [threshold, setThreshold] = useLocalStorageNumber(
    "userThreshold",
    400000,
  );

  // Flea market price type
  const [fleaPriceType, setFleaPriceType] =
    useLocalStorageString<FleaPriceType>(
      FLEA_PRICE_TYPE_KEY,
      "lastLowPrice",
      FLEA_PRICE_TYPES,
    );
  ...
  const [useLastOfferCountFilter, setUseLastOfferCountFilter] =
    useLocalStorageBool(USE_LAST_OFFER_COUNT_FILTER_KEY, false);
