...
async function fetchAndProcessItems() {
  ...
  console.log("[PVP] Sending GraphQL query to tarkov.dev API");
  ...
  if (!response.ok) {
    ...
    console.error("[PVP] API response not OK:", {
      status: response.status,
      statusText: response.statusText,
      error: errorText,
    });
    throw new Error(
      `Failed to fetch PVP data: ${response.status} ${response.statusText}`
    );
  }

  const jsonData: GraphQLResponse = await response.json();
  console.log("[PVP] Received response from API:", {
  ...
  if (jsonData.errors) {
    console.error("[PVP] GraphQL errors:", jsonData.errors);
    throw new Error(jsonData.errors[0]?.message || "Unknown GraphQL error");
  }

  if (!jsonData.data?.items) {
    console.error("[PVP] Invalid data structure:", jsonData);
  ...
  console.log("[PVP] Processed items:", {
